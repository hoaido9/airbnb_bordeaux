...
#### dataset ####
listing <- read_csv('listings.csv', na = c("", "NA", "0", NaN))
...
# Test with 1000 first row of Review dataset
review_demo <- read_csv('review_demo.csv')
text <- review_demo$comments
...
# Sentiment
sentiment=sentiment_by(review_demo$comments)
...
main_body <- dashboardBody(
        tabItems( 
            ...
            # tabItem( # Tab2: Var quanti
            #     tabName = 'tab_quantitative',
            #     fluidRow(
            #         column(3,
            #                selectInput("quantitative",
            #                            "Choisir la variable :",
            #                            choices = c('price', 'minimum_nights','number_of_reviews',
            #                                        'reviews_per_month','availability_365', 'income_monthly', 
            #                                        'calculated_host_listings_count')
            #         )),
            #         column(3,
            #                sliderInput("max_quanti", "Max", min = 0, max = 5000, value = 500)
            #                )
            #     ),
            #     fluidRow(
            #         column(4,
            ...
            #     ),
            #     h4("Distribution"),
            #     plotOutput("histo_quanti")
            # ),
            # tabItem( # Tab3: Var quali
            #     tabName = 'tab_qualitative',
            #     fluidRow(
            #         column(3,
            #                selectInput("qualitative",
            #                            "Choisir la variable :",
            #                            choices = c('neighbourhood_group', 'room_type','highly_available',
            #                                        'freq_review')
            #                )),
            #         column(3, 
            #                h4("Définition"),
            #                 textOutput('definition_quali')
            #         )
            #     ),
            #     h4("Répartition"),
            #     conditionalPanel(
            #         condition = "input.qualitative != 'neighbourhood_group'",
            #         plotOutput("pie_quali")
            #     ),
            #     conditionalPanel(
            #         condition = "input.qualitative == 'neighbourhood_group'",
            #         plotOutput("histo_quali")
            #     )
            # ),
            # tabItem( # Tab4: 2 vars
            #     tabName = 'tab_2vars',
            ...
            #                            choices = c('room_type','highly_available',
            #                                        'freq_review')
            #                )),
            #         column(3,
            #                selectInput("var2",
            ...
            #         column(3,
            #                conditionalPanel(
            #                    condition = "input.var2 == 'price'",
            #                    sliderInput("max_y", "Max", min = 0, max = 3000, value = 500)
            #                ),
            #                conditionalPanel(
            #                    condition = "input.var2 != 'price'",
            #                    sliderInput("max_y", "Max", min = 0, max = 500, value = 100)
            #                )
            #                
            #         )
            #     ),
            #     h4("Graphique"),
            #     plotOutput("graph_2vars")
            # ),
 # Page 2
...
server <- function(input, output) {
    ...
    # # Tab 2: Var quantitative
    # output$definition <- renderText({
    #     col_def[[input$quantitative]]
    # })
    # 
    # output$summary <- renderPrint(
    #     listing %>% 
    #         select(input$quantitative) %>% 
    #         summary()
    # )
    # 
    # df_quanti <- reactive({
    #     listing[listing[[input$quantitative]]<= input$max_quanti,]
    #     })
    # 
    # output$histo_quanti <- renderPlot({
    #         ggplot()+
    #         geom_histogram(aes(x=df_quanti()[[input$quantitative]]), fill = "aquamarine4")+
    #         labs(x=input$quantitative) +
    #         theme_minimal()
    # })
    # 
    # # Tab 3: Var qualitative
    # output$definition_quali <- renderText({
    #     col_def[[input$qualitative]]
    # })
    # 
    # output$histo_quali <- renderPlot({
    #     ggplot()+
    #         geom_bar(aes(x = listing[[input$qualitative]]), fill = "aquamarine4")+
    #         labs(x="", y="", fill=input$qualitative) +
    #         theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))+
    #         theme_minimal()
    # })
    # 
    # output$pie_quali <- renderPlot({
    #     ggplot()+
    #         geom_bar(aes(x = factor(1), fill = listing[[input$qualitative]]), width = 1)+
    #         coord_polar(theta='y')+
    #         labs(x="", y="", fill=input$qualitative) +
    #         theme_minimal()
    # })
    ...
    # # Tab 4: 2 vars
    # df_2var <- reactive({
    #     listing[listing[[input$var2]]<= input$max_y,]
    # })
    # 
    # output$graph_2vars <- renderPlot({
    #     ggplot()+
    #         geom_boxplot(aes(df_2var()[[input$var1]], df_2var()[[input$var2]]), 
    #                      colour='sienna1', outlier.alpha = 0.2, na.rm = T)+
